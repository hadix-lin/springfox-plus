...
@Component
@Order(AFTER_SWAGGER)
class JavaDocExpandedParameterBuilder(private val docStore: DocStore) :
    ExpandedParameterBuilderPlugin {
    override fun apply(context: ParameterExpansionContext) {
        val metadataAccessor =
            readField(context, "metadataAccessor", true) as ModelAttributeParameterMetadataAccessor
        val annotatedElements =
            readField(metadataAccessor, "annotatedElements", true) as List<AnnotatedElement>
